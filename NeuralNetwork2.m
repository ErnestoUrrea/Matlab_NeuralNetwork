classdef NeuralNetwork2 < handle
    ...
    methods
        ...
        function output = forwardPropagation(obj,input)
            ...
            for layer = 2:size(obj.layers,2)
                aux = obj.weights{layer}*obj.activatedNeurons{layer-1}+obj.bias{layer};
                obj.inactiveNeurons{layer} = aux;
                if layer == size(obj.layers,2)
                    obj.activatedNeurons{layer} = obj.softmaxActivation(aux);
                else 
                    obj.activatedNeurons{layer} = obj.reluActivation(aux);
                end
            end
